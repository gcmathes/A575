{
 ...
 newspecid = substr($0,1,12) #Grab the whole line, starting at 1st character, grab 12 characters.
 nlines = $2 
 id = int(newspecid)
 ...
 else if (newspecid * 1.0 == 114.29) molid = 114.001029 #This chunk converts the 
 else if (newspecid * 1.0 == 114.30) molid = 114.001030 #number from newspecid to 
 else if (newspecid * 1.0 == 606.12) molid = 606.012012 #a specific ion id. 
 ...
 if (substr(newspecid,7,3) == ".03") iontype = "IV" #Assign the previous id types
 if (substr(newspecid,7,3) == ".04") iontype = "V " #some string id's based on the 
 if (substr(newspecid,7,3) == ".05") iontype = "VI" #decimals 
 ...
 else if (newspecid * 1.0 < 100 && substr($0,10,3) != "000" && newspecid * 1.0 > 1.5 && (iontype == "II" || iontype == "III" || iontype == "I ")) printf("'%7.4f             '%5.0f%10i\n",id + substr($0,10,3)/1000,ion,nlines) #These two lines print out 
					 #id numbers for the lines. The two lines
					 #print out the id simply if it ends with 000
                                         #or makes it work as a decimal. Also sorts
                                         #this only if it matches I II or III ion
                                         #types.
 ...
 if (id ==  3) type = "LI"
 if (id ==  4) type = "BE"
 if (id ==  5) type = "B " #Depending on the atomic number, assign every id to an
 if (id ==  6) type = "C " #element. 
 if (id ==  7) type = "N "
 if (id ==  8) type = "O "
 if (id ==  9) type = "F "
 if (id == 10) type = "NE"
 ...
 while ((getline < inputfile) && newspecid * 1.0 > 1.5 && (iontype == "II" || iontype == "III" || iontype == "I ")) {
  if (substr($0,1,1) != "#") {  #This makes sure to eliminate commented lines.
    count = count + 1
    specid = substr($0,47,8) * 1.0 #Pushes the count and makes the specid a float...
    if (specid * 1.0 == substr(newspecid,2,8) * 1.0 ) {
     iso2 = substr($0,143,3) #At this point, it starts recording many many columns
     isof = substr($0,146,6) #of data into variables. 
     if((substr(newspecid,10,3)*1.0 != 0 && iso2*1.0 == substr(newspecid,10,3)*1.0 && isof * 1.0 != 0) || (substr(newspecid,10,3)*1.0 == 0 && isof*1.0 == 0) ) {
      ...
      if (vdWast * 1.0 < -1.0 ) vdW = vdWast          #These look like they're fixing
      if (EP1 * 1 < 0) { ep = EP1 * -1; gu = (J2 * 2.0) + 1} #any possible sign error
      else {ep = EP1; gu = (J2 * 2.0) + 1}     #that could have been recorded in the
      if (EP2 * 1 < 0) { EP2 = EP2 * -1}       #columns we just saved. 
...
#       if (iontype ="II") printf("%10.3f %6.3f %7.3f %9.2f %6.1f %9.2e 'x' 'x' \n",lam, ep, gf,vdW, gu, 10^(Rad)-1)
     ...
     }      #This final part prints out a lot of data to the terminal with printf. 
  }
 }  
 count = 0      #When we're finished, end the count and close the input file. 
 close (inputfile)
